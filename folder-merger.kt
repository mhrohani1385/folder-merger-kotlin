...
fun main(args: Array<String>) {
		...
		for(fileIndex in IntRange(0, allFiles1.size - 1)) {
			...
			if(file2.exists()) {
				if(isEqual(file1.toPath(), file2.toPath(), { currentCompareProgress ->
						...
						resultEquals += "equels : ." + file1PathStr + "\n"
				} else {
						renameQueue.add(file1 to file2)
						resultActions +=
							"Name equals but files changed (version creation) : ." + file1PathStr + "\n"
				}
			} else {
				copyQueue.add(file1 to file2)
				resultActions += "Add and Copy : ." + file1PathStr + "\n"
			}
		...
		var out_log = "" 
		out_log += "Actions : \n"
		out_log += resultActions
		out_log += "----------------\n"
		out_log += resultEquals

		File("actions-log.txt").writeText(out_log)

		println("Are you sure to do actions that listed in (actions-log.txt) file in current directory ? (Y/N)")
		val response = readln()
